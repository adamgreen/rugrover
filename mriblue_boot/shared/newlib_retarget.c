...
{
    int result = mriNewLib_SemihostOpen(pFilename, flags, mode);
    if (result < 0)
    {
...
{
    int result = mriNewLib_SemihostRename(pOldFilename, pNewFilename);
    if (result < 0)
    {
        errno = mriNewlib_SemihostGetErrNo();
    }
...
{
    int result = mriNewLib_SemihostUnlink(pFilename);
    if (result < 0)
    {
        errno = mriNewlib_SemihostGetErrNo();
    }
